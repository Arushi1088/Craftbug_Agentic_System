#!/usr/bin/env python3
"""
Test to inspect all elements in the Copilot dialog to find the X button
"""
import asyncio
import time
from excel_web_selenium_only import get_selenium_navigator

async def test_inspect_dialog():
    """Test to inspect all elements in the Copilot dialog"""
    print("🔍 Inspecting Copilot dialog elements...")
                                    ...
                                    if dialogs:
                                        ...
                                        for dialog in dialogs:
                                            try:
                                                ...
                                                # Inspect all elements in the dialog
                                                print(f"    🔍 Inspecting all elements in dialog...")
                                                ...
                                                for j, elem in enumerate(all_elements):
                                                    try:
                                                        ...
                                                        # Only print elements that might be the X button
                                                        if (elem_text in ["×", "✕", "X", "x", "Close", "close"] or
                                                            "close" in aria_label.lower() or
                                                            "dismiss" in aria_label.lower() or
                                                            "close" in class_name.lower() or
                                                            "dismiss" in class_name.lower() or
                                                            "close" in title.lower() or
                                                            "dismiss" in title.lower() or
                                                            tag_name in ["button", "a", "span", "div"]):
                                                            
                                                            print(f"      Element {j}: tag='{tag_name}', text='{elem_text}', class='{class_name}', aria-label='{aria_label}', title='{title}', role='{role}'")
                                                            
                                                            # Try to click this element
                                                            try:
                                                                elem.click()
                                                                print(f"        ✅ Successfully clicked element {j}")
                                                                await asyncio.sleep(2)
                                                                break
                                                            except Exception as e:
                                                                print(f"        ❌ Failed to click element {j}: {e}")
...
if __name__ == "__main__":
    asyncio.run(test_inspect_dialog())
